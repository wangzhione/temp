...
// net.Listener.
func (h HTTP) Serve(hs *http.Server, listener net.Listener) Server {
...
func (h HTTP) ListenAndServe(hs *http.Server) (Server, error) {
	addr := hs.Addr
	if addr == "" {
		if hs.TLSConfig == nil {
			addr = ":http"
...
func ListenAndServe(hs *http.Server, h *HTTP) error {
	if h == nil {
		h = &HTTP{}
	}
	s, err := h.ListenAndServe(hs)
	if err != nil {
		return err
	}

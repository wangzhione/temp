...
func main() {
	flag.Parse()
	gracehttp.Serve(
		&http.Server{Addr: *address0, Handler: newHanlder("Zero  ")},
		&http.Server{Addr: *address1, Handler: newHanlder("First ")},
		&http.Server{Addr: *address2, Handler: newHanlder("Second")},
	)
}

func newHanlder(name string) http.Handler {

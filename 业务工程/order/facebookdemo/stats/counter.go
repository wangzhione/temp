...
func (s *SimpleCounter) FullKey() string { return s.Key }

// GetValues is part of the Counter interface
func (s *SimpleCounter) GetValues() []float64 { return s.Values }

// AddValues is part of the Counter interface
func (s *SimpleCounter) AddValues(args ...float64) { s.Values = append(s.Values, args...) }

// GetType is part of the Counter interface
func (s *SimpleCounter) GetType() Type { return s.Type }
...
func (s *SimpleCounter) Aggregate() map[string]float64 {
	switch s.Type {
	case AggregateAvg:
		return map[string]float64{s.Key: Average(s.Values)}
	case AggregateSum:
		return map[string]float64{s.Key: Sum(s.Values)}
	case AggregateHistogram:
		histogram := map[string]float64{s.Key: Average(s.Values)}

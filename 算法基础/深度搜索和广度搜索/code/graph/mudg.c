...
/*
 * 邻接矩阵图表示的无向图 (Matrix Undirected Graph) 
 * 
 * cc -g -O2 -Wall -Wextra -Wno-sign-compare -o mudg mudg.c
 */
...
static struct graph * create_example_graph(void) {
    ...
    for (int i = 0; i < (sizeof edges / sizeof *edges); i++) {
        ...
        graph_set_position(g, head, tail);
    }
...
void graph_dfs(struct graph * g) {
    ...
    // 业务处理
    printf("Matrix Undirected Graph DFS:");
...
int main(void) {
    ...
    // 输出图
    printf("Matrix Undirected Graph:\n");
    /*  A - F - G - E
     *  | \
     *  C - D
     *  |
    ...
    // 输出边和邻接矩阵关系
    printf("Matrix Undirected Graph vertex:");
    for (int i = 0; i < g->num; i++) {
        printf(" %c", g->vertex[i]);
    }
    putchar('\n');

    printf("Matrix Undirected Graph matrix:\n");
    for (int i = 0; i < g->num; i++) {

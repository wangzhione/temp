...
// 将 node 链接到 list 的末尾
static struct graph_node * graph_insert_last(struct graph_node * list, struct graph_node * node) {
    if (!list)
        list = node;
    else {
...
static struct graph * create_example_graph(void) {
    ...
    for (int i = 0; i < LEN(edges); i++) {
        ...
        g->vertex[v].edge = graph_insert_last(g->vertex[v].edge, nodew);
        g->vertex[w].edge = graph_insert_last(g->vertex[w].edge, nodev);
    }
